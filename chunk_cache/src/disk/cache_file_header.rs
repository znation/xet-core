...
///
/// format:
/// (chunk_byte_indicies length n)
/// (
///     chunk_byte_indicies[0]
///     chunk_byte_indicies[1]
///     chunk_byte_indicies[2]
///     ...
///     chunk_byte_indicies[n - 1]
/// )
...
impl CacheFileHeader {
    ...
    pub fn header_len(&self) -> usize {
        (self.chunk_byte_indicies.len() + 1) * size_of::<u32>()
    }
    ...
    pub fn deserialize<R: Read + Seek>(reader: &mut R) -> Result<Self, ChunkCacheError> {
        ...
        let mut chunk_byte_indicies: Vec<u32> =
            Vec::with_capacity(chunk_byte_indicies_len as usize);
        for i in 0..chunk_byte_indicies_len {
            let idx = read_u32(reader)?;
            if i == 0 && idx != 0 {
                return Err(ChunkCacheError::parse("first byte index isn't 0"));
            } else if !chunk_byte_indicies.is_empty() && chunk_byte_indicies.last().unwrap() >= &idx
            {
        ...
        Ok(Self::new(chunk_byte_indicies))
    }

    pub fn serialize<W: Write>(&self, writer: &mut W) -> Result<(), std::io::Error> {
        write_u32(writer, self.chunk_byte_indicies.len() as u32)?;
        write_u32s(writer, &self.chunk_byte_indicies)?;
        Ok(())
    }
}
